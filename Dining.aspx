        ...
        <section id="menuShowcase" class="py-5" style="background-color: #faf8f5;">
            <div class="container py-4 px-3 px-lg-5">
                ...
                <div class="row g-4" id="menuItemsContainer">
                    ...
                    <!-- Dynamic Database Menu Items Repeater -->
                    <asp:PlaceHolder ID="phNoDishes" runat="server" Visible="false">
                        <div class="col-12 text-center py-5">
                            <i class="bi bi-cup-hot fs-1 text-gold mb-2 d-block"></i>
                            <h4 class="font-serif text-dark">Menu Currently Being Updated</h4>
                            <p class="text-muted">Our culinary selections will be available shortly.</p>
                        </div>
                    </asp:PlaceHolder>

                    <asp:Repeater ID="rptDiningMenu" runat="server">
                        <ItemTemplate>
                            <div class='col-lg-6 menu-item-col <%# Eval("Category").ToString().ToLower() == "kebabs" ? "" : "d-none" %>'
                                data-menu-cat='<%# Eval("Category").ToString().ToLower() %>' data-aos="fade-up">
                                <div class="menu-dish-card">
                                    <div class="menu-dish-thumb">
                                        <img src='<%# ResolveUrl("~/" + Eval("ImageUrl")) %>' alt='<%# Eval("ItemName") %>'
                                            class="menu-dish-img" onerror="this.src='images/food-paneer-tikka.jpg';" />
                                    </div>
                                    <div class="menu-dish-content">
                                        <div class="d-flex justify-content-between align-items-baseline mb-1">
                                            <h5 class="dish-title"><%# Eval("ItemName") %></h5>
                                            <span class="dish-price">&#8377;<%# Convert.ToDecimal(Eval("Price")).ToString("N0") %></span>
                                        </div>
                                        <div class="d-flex flex-wrap gap-1 align-items-center mb-1">
                                            ...
                                            <%# !string.IsNullOrEmpty(Eval("Badge") as string)
                                                ? "<span class='badge-chef-sig'><i class='bi bi-award-fill text-warning'></i> " + Eval("Badge") + "</span>"
                                                : "" %>
                                        </div>
                                        <p class="dish-desc"><%# Eval("Description") %></p>
                                        <%# !string.IsNullOrEmpty(Eval("PairingNote") as string)
                                            ? "<div class='dish-pairing-note'><i class='bi bi-sparkles text-warning'></i> " + Eval("PairingNote") + "</div>"
                                            : "" %>
                                    </div>
                                </div>
                            </div>
